...
def init_dvc(**context):
    ...
    try:
        ...
        subprocess.run(
            ["dvc", "remote", "add", "-f", "-d", remote_name, s3_url], check=True
        )
        ...
        subprocess.run(
            ["dvc", "config", f"remote.{remote_name}.endpointurl", endpoint], check=True
        )
        logging.info(
            f"DVC remote '{remote_name}' configured successfully with endpoint {endpoint}."
        )

        try:
            result = subprocess.run(
                ["dvc", "pull", "--force"], capture_output=True, text=True, check=True
            )
...
def load_data(**kwargs):
    ...
    try:
        ...
        result = subprocess.run(
            ["dvc", "pull", "--force"], capture_output=True, text=True
        )
        if result.returncode != 0:
            logging.error(
                f"DVC pull failed: stdout={result.stdout}, stderr={result.stderr}"
            )
...
def train_model(**kwargs):
    ...
    metadata = {"timestamp": str(datetime.now()), "description": "Trained ML model"}
    ...
    try:
        subprocess.run(
            ["dvc", "add", "models/model.pkl", "models/metrics.json"], check=True
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            subprocess.run(
                ["git", "commit", "-m", "Updated model and metrics"], check=True
            )
