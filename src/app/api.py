...
def load_model_and_metrics(app: FastAPI):
    ...
    if USE_DVC and int(USE_DVC) == 1:
        try:
            subprocess.run(
                ["dvc", "pull", "--force", "models/model.pkl.dvc"],
                check=True,
                cwd=PROJECT_ROOT,
            )
        ...
        try:
            subprocess.run(
                ["dvc", "pull", "models/metrics.json.dvc"], check=True, cwd=PROJECT_ROOT
            )
...
def predict(request: Request, data: WineFeatures) -> dict:
    ...
    if not all(key in mapped_data for key in expected_features):
        raise HTTPException(
            status_code=400, detail="Missing or invalid features after mapping"
        )
...
def model_info(request: Request) -> dict:
    ...
    try:
        info = {
            ...
            "classes": (
                getattr(model, "classes_", None).tolist()
